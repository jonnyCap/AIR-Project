...
class BusinessIdeaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bert_encodings = torch.tensor(bert_encodings, dtype=torch.float).to(self.device)
...
# Ranking model to predict the score based on encoded idea and stock data
class EnhancedRankingModel(nn.Module):
    ...
    def forward(self, idea_encoding, stock_performance):
        ...
        # Compute the score
        score = self.combined_fc(flattened_features).squeeze()
...
if __name__ == "__main__":
    ...
    dataset = BusinessIdeaDataset(
        csv_file="Dataset/Data/padded_entire_final_dataset.csv"
    )
